...
export default function CartPage() {
  ...
  useEffect(() => {
    fetchWallet('user-id') // Replace with real user ID from session
      .then(setWallet)
  ...
  const handleCheckout = async () => {
    ...
    // Simulate API call (hold wallet, create order)
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
        ...
        {state.items.map((item) => (
          <div key={item.menuItemId} className="flex justify-between items-center p-4 bg-white rounded-lg shadow">
            <div>
              ...
              <p className="text-gray-600">Qty: {item.quantity} x ${item.price / 100}</p>
              <p className="text-sm text-gray-500">Subtotal: ${(item.quantity * item.price / 100).toFixed(2)}</p>
            </div>
            <div className="flex space-x-2">
              ...
              <button
                onClick={() => dispatch({ type: 'ADD_ITEM', payload: { menuItemId: item.menuItemId, name: item.name, price: item.price } })}
                className="bg-gray-200 px-2 py-1 rounded"
              >
                +
              </button>
      ...
      <div className="bg-gray-100 p-4 rounded-lg">
        <div className="flex justify-between text-lg font-bold">
          ...
          <span>Balance: ${wallet.balance / 100} (Hold: ${wallet.pendingHold / 100})</span>
        </div>
      ...
      <div className="flex space-x-4">
        ...
        <Link href="/wallet" className="flex-1 bg-blue-500 text-white py-3 rounded-lg text-center font-semibold hover:bg-blue-600">
          Top Up Wallet
        </Link>
      </div>
      <Link href="/" className="block text-center text-blue-500 hover:underline">Continue Shopping</Link>
    </div>
  );
}